...
void JsonValue::erase(JsonValue value) { this->val->EraseMember(*value.val); }
...
JsonDocument::JsonDocument(FileHandle& file) {
    rapidjson::Document* doc = new rapidjson::Document();
    doc->Parse(file.syncRead(0, file.getFileSize()).cString());
...
void JsonDocument::save(FileHandle handle) {
    rapidjson::StringBuffer buffer;
    rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
    this->document->doc->Accept(writer);

    handle.syncWrite(buffer.GetString());
}
